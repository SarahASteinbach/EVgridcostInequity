function [nodes,charging_nodes, house_loads, no_pvs, pv_kWs] = assembleNodes4(agglomeration,time_of_year,pv_potential,pv_penetration, BESS_data,EV_penetration,charger_available,H16,car_segment_distribution, car_data, samples, speed, timestep_size, no_timesteps,initial_EV_socs,initial_BESS_socs,house_scaling_factor)
%ASSEMBLENODES 
% Params
...
% col 8: Self consumed energy without battery
% col 9: Self consumed energy with battery
% charging nodes,  house loads and no PVs and overall kWs installed
    ...
    for k=1:no_houses(i)
        ...
        if no_households(house_counter) == 1
            if rand < pv_penetration.single_house*pv_house_factor
                %pv_factor = 10/pv_potential.single_house;
                pv_factor = pv_potential.single_house/10;
                no_pvs=no_pvs+1;
                pv_kWs=pv_kWs + pv_factor;
            end
        elseif no_households(house_counter) == 2
            if rand < pv_penetration.double_house*pv_house_factor
                %pv_factor = 10/pv_potential.double_house;
            ...
            if rand < pv_penetration.multi_house*pv_house_factor
                %pv_factor = 10/pv_potential.multi_house;
        ...
        %Self consumption per house: either PV + storage or
        %house load + EV load
        house_loads{i,k,8} = min(house_loads{i,k,4}, -house_loads{i,k,3});
        house_loads{i,k,9} = min(house_loads{i,k,4} + house_loads{i,k,6}, -house_loads{i,k,3});
...
% adjusted for 8th element (self-consumption)
% create node profiles by summing up over all houses at a node
nodes = cell(no_nodes,9);
for i = 1:no_nodes
    for k=1:no_houses(i)
        for j = 1:9
