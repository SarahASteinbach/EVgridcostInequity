function [handle] = plot_mpc(mpc, pfresult, varargin)
...
% Generate a color map with 10 values
cmap = colormap(summer(10));
...
AllHandles = [];
    ...
    for ThisVoltageLevel = 1:size(DistinctVoltageLevels,1) %Run through and draw all the branches first
        ...
        BranchesAtThisLevel = find(BranchVoltages == DistinctVoltageLevels(ThisVoltageLevel));
            ...
            BranchVoltage = pfresult.bus(BranchesAtThisLevel,8);
            minBranchVoltage = min(BranchVoltage)*10;
            maxBranchVoltage = max(BranchVoltage)*10;

            LineSegments = [Layout(FromBuses,:), Layout(ToBuses,:)];
            CurrentThickness = ThisVoltageLevel/size(DistinctVoltageLevels,1) * 3;
            
            % Color each line segment according to voltage
            for l=1:size(LineSegments)
                voltage = BranchVoltage(l)*10;
                % map distribution of voltages from min-max towards 1-10
                if voltage == maxBranchVoltage
                    cmap_idx = 10;
                else
                    cmap_idx = 11-floor(((maxBranchVoltage - voltage) * 9/(maxBranchVoltage-minBranchVoltage)+1));
                end
                ThisHandle(l) = line([LineSegments(l,1),LineSegments(l,3)]' , [LineSegments(l,2), LineSegments(l,4)]', 'color', cmap(cmap_idx,:), 'LineWidth', CurrentThickness*1.5, 'DisplayName', [int2str(DistinctVoltageLevels(ThisVoltageLevel)), ' kV'] );
            end
       %ThisHandle = line([LineSegments(:,1),LineSegments(:,3)]' , [LineSegments(:,2), LineSegments(:,4)]', 'color', cmap(BranchVoltageValue,:), 'LineWidth', CurrentThickness*1.5, 'DisplayName', [int2str(DistinctVoltageLevels(ThisVoltageLevel)), ' kV'] );
